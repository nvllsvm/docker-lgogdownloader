...
RUN \
    # Build deps
    apk add --no-cache help2man cmake make g++ curl-dev liboauth-dev jsoncpp-dev tinyxml2-dev rhash-dev boost-dev && \
    # Run Deps
    apk add --no-cache boost boost-program_options boost-iostreams boost-date_time jsoncpp liboauth rhash tinyxml2 curl shadow su-exec && \
